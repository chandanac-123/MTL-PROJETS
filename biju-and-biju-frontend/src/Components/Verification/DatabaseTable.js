...
import { fetchExcelData } from "../../Functions/helper";
import DatabaseEditModal from "./DatabaseEditModal";
import { get_data } from "../../Store/common/commonSlice";
import { database_list_excel } from "../../Api/ReportSubmittedApis";
...
export default function DatabaseTable(props) {
  ...
  const getItemValue = (item, name, fi_type = "") => {
    ...
    if (
      ...
      result = item.product?.vendor_code[0]?.vendor_code.toLowerCase();
      ...
      result = item.addresses.filter((p) => p.fi_type === fi_type)[0]
        ? item.addresses.filter((p) => p.fi_type === fi_type)[0]
          ...
          : "No"
        : "";
    } else if (name === "customer_number") {
      result = item.customer_number;
    } else if (name === "applicant_type") {
      result = item.applicant_type;
    } else if (
      name === "rv_report_recieved_delay" ||
      name === "bv_report_recieved_delay" ||
      name === "pv_report_recieved_delay" ||
      name === "pd_report_recieved_delay"
    ) {
      result = item.addresses.filter((p) => p.fi_type === fi_type)[0]
        ?.verification_address[0]?.recievedDelay;
    } else if (
      name === "rv_submitted_delay" ||
      name === "bv_submitted_delay" ||
      name === "pv_submitted_delay" ||
      name === "pd_submitted_delay"
    ) {
      result = item.addresses.filter((p) => p.fi_type === fi_type)[0]
        ?.verification_address[0]?.submitted_delay;
    }
    return result;
  };
  ...
  const handleClick = () => {
    // exportToExcel(fetchExcelData(props.data.results), "database")
    let url = database_list_excel + "?";
    let from_date = "";
    let to_date = "";
    ...
    if (from_date) {
      if (props?.state.date_type == "fi_date") {
        url = url + `&addresses__fi_date_time__date__gte=${from_date}`;
      } else {
        url =
          url +
          `&addresses__verification_address__submitted_at__date__gte=${from_date}`;
      }
    }

    if (to_date) {
      if (props?.state.date_type == "submitted_date") {
        url =
          url +
          `&addresses__verification_address__submitted_at__date__lte=${to_date}`;
      } else {
        url = url + `&addresses__fi_date_time__date__lte=${to_date}`;
      }
    }
    if (props?.state.executives && props?.state.executives !== "all") {
      url =
        url +
        `&addresses__verification_address__assigned_verification_users__field_agent__id=${props?.state.executives}`;
    }
    ...
    <>
      {isMobileScreen ? (
        <DatabaseTableAccordion
          ...
          sameVendorsCheck={props?.sameVendorsCheck}
          state={props?.state}
            ...
            <TableMain aria-label="sticky table">
              <TableHeadSection>
                <TableRow>
                      ...
                      <SortIcon
                        onClick={() =>
                          handleSorting(
                            "customer_number",
                            "number",
                            "customer_number_ascending",
                            "customer_number_decsending",
                            ""
                          )
                        }
                        className="mb"
                  ...
                  <TableCellDiv className="head">
                    <THDiv>
                      <p>RV Report Received Delay</p>{" "}
                      <SortIcon
                        onClick={() =>
                          handleSorting(
                            "rv_report_recieved_delay",
                            "delay",
                            "rv_report_recieved_delay_ascending",
                            "rv_report_recieved_delay_decsending",
                            "RV"
                          )
                        }
                        className="mb"
                  ...
                  <TableCellDiv className="head">
                    <THDiv>
                      <p>BV Report Received Delay</p>{" "}
                      <SortIcon
                        onClick={() =>
                          handleSorting(
                            "bv_report_recieved_delay",
                            "delay",
                            "bv_report_recieved_delay_ascending",
                            "bv_report_recieved_delay_decsending",
                            "BV"
                          )
                        }
                        className="mb"
                  ...
                  <TableCellDiv className="head">
                    <THDiv>
                      <p>PV Report Received Delay</p>{" "}
                      <SortIcon
                        onClick={() =>
                          handleSorting(
                            "pv_report_recieved_delay",
                            "delay",
                            "pv_report_recieved_delay_ascending",
                            "pv_report_recieved_delay_decsending",
                            "PV"
                          )
                        }
                        className="mb"
                  ...
                  <TableCellDiv className="head">
                    <THDiv>
                      <p>PD Report Received Delay</p>{" "}
                      <SortIcon
                        onClick={() =>
                          handleSorting(
                            "pd_report_recieved_delay",
                            "delay",
                            "pd_report_recieved_delay_ascending",
                            "pd_report_recieved_delay_decsending",
                            "PD"
                          )
                        }
                        className="mb"
                  ...
                  <TableCellDiv className="head">
                    <THDiv>
                      <p>RV Submitted Delay</p>{" "}
                      <SortIcon
                        onClick={() =>
                          handleSorting(
                            "rv_submitted_delay",
                            "delay",
                            "rv_submitted_delay_ascending",
                            "rv_submitted_delay_decsending",
                            "RV"
                          )
                        }
                        className="mb"
                  ...
                  <TableCellDiv className="head">
                    <THDiv>
                      <p>BV Submitted Delay</p>{" "}
                      <SortIcon
                        onClick={() =>
                          handleSorting(
                            "bv_submitted_delay",
                            "delay",
                            "bv_submitted_delay_ascending",
                            "bv_submitted_delay_decsending",
                            "BV"
                          )
                        }
                        className="mb"
                  ...
                  <TableCellDiv className="head">
                    <THDiv>
                      <p>PV Submitted Delay</p>{" "}
                      <SortIcon
                        onClick={() =>
                          handleSorting(
                            "pv_submitted_delay",
                            "delay",
                            "pv_submitted_delay_ascending",
                            "pv_submitted_delay_decsending",
                            "PV"
                          )
                        }
                        className="mb"
                  ...
                  <TableCellDiv className="head">
                    <THDiv>
                      <p>PD Submitted Delay</p>{" "}
                      <SortIcon
                        onClick={() =>
                          handleSorting(
                            "pd_submitted_delay",
                            "delay",
                            "pd_submitted_delay_ascending",
                            "pd_submitted_delay_decsending",
                            "PD"
                          )
                        }
                        className="mb"
                ...
                {props?.data?.results?.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCellDiv className="item">
                        {
                          row.addresses.filter(
                            (p) => p.fi_type === "PD" || "RV" || "BV" || "PV"
                          )[0]?.verification_address[0]?.assign_verification[0]
                            ?.vendorcode
                        }
                      </TableCellDiv>
                      <TableCellDiv className="item">
                      ...
                      <TableCellDiv className="item">
                        {rv?.verification_address[0]?.recievedDelay}
                      </TableCellDiv>
                      <TableCellDiv className="item">
                        {bv?.verification_address[0]?.recievedDelay}
                      </TableCellDiv>
                      <TableCellDiv className="item">
                        {pv?.verification_address[0]?.recievedDelay}
                      </TableCellDiv>
                      <TableCellDiv className="item">
                        {pd?.verification_address[0]?.recievedDelay}
                      </TableCellDiv>
                      ...
                      <TableCellDiv className="item">
                        {rv?.verification_address[0]?.submitted_delay}
                      </TableCellDiv>
                      <TableCellDiv className="item">
                        {bv?.verification_address[0]?.submitted_delay}
                      </TableCellDiv>
                      <TableCellDiv className="item">
                        {pv?.verification_address[0]?.submitted_delay}
                      </TableCellDiv>
                      <TableCellDiv className="item">
                        {pd?.verification_address[0]?.submitted_delay}
                      </TableCellDiv>
                      ...
                      <TableCellDiv className="item">
                        {rv?.verification_address[0]?.image_1_latlong?.[0]
                          ? `Image1- Lat: ${rv?.verification_address[0]?.image_1_latlong?.[0]?.latitude}, Long: ${rv?.verification_address[0]?.image_1_latlong?.[0]?.longitude}`
                          : ""}
                        <br />
                        {rv?.verification_address[0]?.image_2_latlong?.[0]
                          ? `Image2- Lat: ${rv?.verification_address[0]?.image_2_latlong?.[0]?.latitude}, Long: ${rv?.verification_address[0]?.image_2_latlong?.[0]?.longitude}`
                          : ""}
                        <br />
                        {rv?.verification_address[0]?.image_3_latlong?.[0]
                          ? `Image3- Lat: ${rv?.verification_address[0]?.image_3_latlong?.[0]?.latitude}, Long: ${rv?.verification_address[0]?.image_3_latlong?.[0]?.longitude}`
                          : ""}
                        <br />
                      </TableCellDiv>
                      <TableCellDiv className="item">
                        {bv?.verification_address[0]?.image_1_latlong?.[0]
                          ? `Image1- Lat: ${bv?.verification_address[0]?.image_1_latlong?.[0]?.latitude}, Long: ${bv?.verification_address[0]?.image_1_latlong?.[0]?.longitude}`
                          : ""}{" "}
                        <br />
                        {bv?.verification_address[0]?.image_2_latlong?.[0]
                          ? `Image2- Lat: ${bv?.verification_address[0]?.image_2_latlong?.[0]?.latitude}, Long: ${bv?.verification_address[0]?.image_2_latlong?.[0]?.longitude}`
                          : ""}{" "}
                        <br />
                        {bv?.verification_address[0]?.image_3_latlong?.[0]
                          ? `Image3- Lat: ${bv?.verification_address[0]?.image_3_latlong?.[0]?.latitude}, Long: ${bv?.verification_address[0]?.image_3_latlong?.[0]?.longitude}`
                          : ""}{" "}
                        <br />
                      </TableCellDiv>
                      <TableCellDiv className="item">
                        {pv?.verification_address[0]?.image_1_latlong?.[0]
                          ? `Image1- Lat: ${pv?.verification_address[0]?.image_1_latlong?.[0]?.latitude}, Long: ${pv?.verification_address[0]?.image_1_latlong?.[0]?.longitude}`
                          : ""}{" "}
                        <br />
                        {pv?.verification_address[0]?.image_2_latlong?.[0]
                          ? `Image2- Lat: ${pv?.verification_address[0]?.image_2_latlong?.[0]?.latitude}, Long: ${pv?.verification_address[0]?.image_2_latlong?.[0]?.longitude}`
                          : ""}{" "}
                        <br />
                        {pv?.verification_address[0]?.image_3_latlong?.[0]
                          ? `Image3- Lat: ${pv?.verification_address[0]?.image_3_latlong?.[0]?.latitude}, Long: ${pv?.verification_address[0]?.image_3_latlong?.[0]?.longitude}`
                          : ""}{" "}
                        <br />
                      </TableCellDiv>

                      <TableCellDiv className="item">
                        {pd?.verification_address[0]?.image_1_latlong?.[0]
                          ? `Image1- Lat: ${pd?.verification_address[0]?.image_1_latlong?.[0]?.latitude}, Long: ${pd?.verification_address[0]?.image_1_latlong?.[0]?.longitude}`
                          : ""}{" "}
                        <br />
                        {pd?.verification_address[0]?.image_2_latlong?.[0]
                          ? `Image2- Lat: ${pd?.verification_address[0]?.image_2_latlong?.[0]?.latitude}, Long: ${pd?.verification_address[0]?.image_2_latlong?.[0]?.longitude}`
                          : ""}{" "}
                        <br />
                        {pd?.verification_address[0]?.image_3_latlong?.[0]
                          ? `Image3- Lat: ${pd?.verification_address[0]?.image_3_latlong?.[0]?.latitude}, Long: ${pd?.verification_address[0]?.image_3_latlong?.[0]?.longitude}`
                          : ""}{" "}
                        <br />
                      </TableCellDiv>
                    </TableRow>
...
export const TableHeadSection = styled(TableHead)`
  position: sticky;
  top: 0;
  z-index: 99;
