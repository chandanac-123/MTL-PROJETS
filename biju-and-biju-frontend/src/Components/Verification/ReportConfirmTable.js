...
import ReportConfirmTableAccordion from "./ReportConfirmTableAccordion";

export default function ReportConfirmTable(props) {
  ...
  const handleCheckBox = (checked, val, data) => {
    let addressIds = data?.addresses.map((ids) => ids.id).flat();
    ...
    props.setAddress_ids(addressIds);
  };

  const handleClick = () => {
    // exportToExcel(reportEditConfirmExcelData(props.data.results),"report_confirm")
                    ...
                    >
                      <TableCell padding="checkbox">
                        <Checkbox
                          ...
                          onChange={(e) =>
                            handleCheckBox(e.target.checked, row.id, row)
                          }
                          inputProps={{
