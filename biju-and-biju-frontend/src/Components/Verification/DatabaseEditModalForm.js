...
import { database_list, v2_employees } from "../../Api/ReportSubmittedApis";
...
function DatabaseEditModalForm(props) {
    ...
    if (props.data) {
      setState((prevState) => {
        return {
          ...
          billing_location_data: sortByName(
            props.data?.product?.billing_location || []
          ),
  ...
  const handleSubmit = async () => {
    ...
    let param = {
      values: data,
      url: database_list + props.data.id + "/",
    };
    dispatch(patch_data(param))
      .then((res) => {
        ...
        if (result?.status === 200) {
          handleClose();
          alertSuccessFun("Database Report Edit successfully");
          setLoading(false);
        }
      })
      .catch((error) => {
    ...
    <>
      <FormContainer>
        <HeadSection>
          ...
          <SubHead>
            <Title>Database Edit({state.application_no})</Title>
          </SubHead>
        ...
        <FormSection>
          <ItemRow>
            ...
            <ItemSection>
              ...
              {/* <TextFieldItem width="160px" textField="Text"  handleChange={handleChange} name="application_no" value={state.application_no} /> */}
            </ItemSection>
          </ItemRow>
          <ItemRow>
            <LabelSection>
              <Label className="secondary">Prdoduct Name</Label>
            </LabelSection>
            <ItemSection>
        ...
        <ButtonSection>
          <ButtonItem
            ...
            fsize="16px"
            fweight="500"
            fpadding={isMobileScreen ? "6px 50px" : "6px 120px"}
            bradius="8px"
          />
        </ButtonSection>
      </FormContainer>
...
export default DatabaseEditModalForm;
