...
import AddNegativeRemarksModalForm from "./AddNegativeRemarksModalForm";
...
export default function NegativeRemarksTable(props) {
  ...
  const handleDelete = (row) => {
    const url = negative_remark + row.id + "/";
    Swal.fire({
      title: "Are you sure?",
  ...
  const handleClick = () => {
    let result = props?.data?.results?.map((item, index) => {
      return {
        SI_NO: (props.page - 1) * 10 + index + 1,
        Description: item.description,
      };
    });
    exportToExcel(result, "Negative Remarks");
  };
  ...
  return (
    <>
      ...
      <TableFooter>
        <ExportExcel
          fileName="negative_remarks.xlsx"
          source="NEGATIVE_REMARK"
