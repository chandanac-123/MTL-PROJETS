...
import { timeDate, exportToExcel } from "../../Functions/utils";
...
export default function OfficeWisePendingDetailsTable(props) {
  ...
  const handleClick = () => {
    exportToExcel(
      fetchExcelReportReceived(props.data.results),
      "Report Recieved"
    );
  };
  return (
    <>
      ...
      <TableFooter>
        <ExportExcel
          fileName="Report_Recieved.xlsx"
          source="OFFICE_WISE_TOTAL_PENDING"
...
const MainContainer = styled(TableContainer)`
  ...
  max-width: ${({ isCollapse }) =>
    isCollapse ? "1180px !important" : "1354px !important"};
  overflow-x: scroll;
  // ::-webkit-scrollbar {
  //     display: none;
  //   }
