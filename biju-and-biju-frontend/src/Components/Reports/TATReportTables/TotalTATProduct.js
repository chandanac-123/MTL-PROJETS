...
import TotalTATProductAccordion from "./TotalTATProductAccordion";
...
export default function TotalTATProduct(props) {
  ...
  useEffect(() => {
    async function fetchMyAPI() {
      ...
      if ((start !== null) & (end !== null)) {
        url =
          url +
          `&total_tat_duration__range=${start},${end}&verification_address__verification__product__id=${id}`;
      } else if (start !== null && end === null) {
        url =
          url +
          `&total_tat_duration__gte=${start}&verification_address__verification__product__id=${id}`;
      } else if ((start === null) & (end === null)) {
  ...
  const handleClick = () => {
    exportToExcel(props.data.results, "TotalTATProduct");
  };
  ...
  return (
    <>
                  ...
                  props?.data?.results.map((row, index) => (
                    ...
                    >
                      ...
                      <TableCellDiv className="item">
                        {" "}
                        <LinkItem
                          onClick={() => handleOpen(row.id, 14400, 21600)}
                        >
                          {row.four_six}
                        </LinkItem>
                      </TableCellDiv>
                      <TableCellDiv className="item">
                        {" "}
                        <LinkItem
                          onClick={() => handleOpen(row.id, 21600, 28800)}
                        >
                          {row.six_eight}
                        </LinkItem>
                      </TableCellDiv>
                      ...
                      {/* <TableCellDiv className='item'> <p>{row.total}</p></TableCellDiv> */}
                    </TableRow>
                  ))}
              </TableBody>
            </TableMain>
      ...
      <TableFooter>
        <ExportExcel
          fileName="TotalTATProduct.xlsx"
          source="TOTAL_TAT_PRODUCT"
