...
import AssignTATDistrictAccordion from "./AssignTATDistrictAccordion";
import { ExportExcel } from "../../ExportExcel";
import { getTatDetailModalStyle } from "../../../common/utils";
...
export default function AssignTATDistrict(props) {
  ...
  useEffect(() => {
    async function fetchMyAPI() {
      ...
      if ((start !== null) & (end !== null)) {
        url =
          url +
          `&assigned_duration__range=${start},${end}&verification_address__adress_district__id=${id}`;
      } else if (start !== null && end === null) {
        url =
          url +
          `&assigned_duration__gte=${start}&verification_address__adress_district__id=${id}`;
      } else if ((start === null) & (end === null)) {
  ...
  const handleClick = () => {
    exportToExcel(props.data.results, "AssignTATDistrict");
  };

  const getItemValue = (item, name, fi_type = "") => {
    let result = "";
    if (name === "district") result = item.name.toUpperCase();
    else if (name === "0-10") result = item.zero_ten;
    else if (name === "10-15") result = item.ten_fifteen;
    else if (name === ">15") result = item.fifteen_greater;
  ...
  const searchParams = {
    assigned_duration__range:
      start !== null && end !== null ? `${start || 0},${end || 0}` : undefined,
    assigned_duration__gte: start !== null && end === null ? start : undefined,
    ...
    <>
      {isMobileScreen ? (
        <AssignTATDistrictAccordion
          ...
          totalPage={totalPage}
          searchParamsForParent={props.searchParams}
        ...
        <>
          <MainContainer component={Paper}>
                ...
                {props?.data?.results?.length > 0 &&
                  props?.data?.results.map((row, index) => (
                    ...
                    >
                      ...
                      <TableCellDiv className="item">
                        {" "}
                        <LinkItem onClick={() => handleOpen(row.id, 600, 900)}>
                          {row.ten_fifteen}
                        </LinkItem>
                      </TableCellDiv>
                      <TableCellDiv className="item">
                        {" "}
                        <LinkItem onClick={() => handleOpen(row.id, 900)}>
                          {row.fifteen_greater}
                        </LinkItem>
                      </TableCellDiv>
                      <TableCellDiv className="item">
            ...
            <Modal
              ...
              hideBackdrop={true}
            >
              <Box sx={getTatDetailModalStyle(isMobileScreen)}>
                <TATDetails
                  name="AssignTATDistrictDetails"
                  setOpen={setOpen}
                  handleClose={handleClose}
                  data={detail_data}
                  count={totalPage}
      ...
      <TableFooter>
        <ExportExcel
          fileName="AssignTATDistrict.xlsx"
          source="ASSIGN_TAT"
