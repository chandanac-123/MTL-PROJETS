...
import AssignTATProductAccordion from "./AssignTATProductAccordion";
...
export default function AssignTATProduct(props) {
  ...
  useEffect(() => {
    async function fetchMyAPI() {
      ...
      if ((start !== null) & (end !== null)) {
        url =
          url +
          `&verification_address__allocation_duration__range=${start},${end}&verification_address__verification__product__id=${id}`;
      } else if (start !== null && end === null) {
        url =
          url +
          `&verification_address__allocation_duration__gte=${start}&verification_address__verification__product__id=${id}`;
      } else if ((start === null) & (end === null)) {
  ...
  const getItemValue = (item, name, fi_type = "") => {
    let result = "";
    if (name === "vendor_name")
      result = item.vendor?.employee_name.toUpperCase();
    else if (name === "product_name") result = item.product_name.toUpperCase();
  ...
  const searchParams = {
    verification_address__allocation_duration__range:
      start !== null && end !== null ? `${start || 0},${end || 0}` : undefined,
    verification_address__allocation_duration__gte:
  ...
  return (
    <>
      ...
      <TableFooter>
        <ExportExcel
          fileName="AssignTATProduct.xlsx"
          source="ALLOCATION_TAT"
