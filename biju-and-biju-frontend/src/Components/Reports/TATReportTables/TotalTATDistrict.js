...
export default function TotalTATDistrict(props) {
  ...
  const searchParams = {
    total_tat_duration__range:
      start !== null && end !== null ? `${start || 0},${end || 0}` : undefined,
    total_tat_duration__gte: start !== null && end === null ? start : undefined,
  ...
  useEffect(() => {
    async function fetchMyAPI() {
      ...
      if ((start !== null) & (end !== null)) {
        url =
          url +
          `&total_tat_duration__range=${start},${end}&verification_address__adress_district__id=${id}`;
      } else if (start !== null && end === null) {
        url =
          url +
          `&total_tat_duration__gte=${start}&verification_address__adress_district__id=${id}`;
      } else if ((start === null) & (end === null)) {
  ...
  const handleClick = () => {
    exportToExcel(props.data.results, "TotalTATDistrict");
  };

  const getItemValue = (item, name, fi_type = "") => {
    let result = "";
    if (name === "district") result = item.name.toUpperCase();
    else if (name === "0-2") result = item.zero_two;
    else if (name === "2-3") result = item.two_three;
    else if (name === "3-4") result = item.three_four;
    else if (name === "4-6") result = item.four_six;
    else if (name === "6-8") result = item.six_eight;
    else if (name === ">8") result = item.eight_greater;
  ...
  return (
    <>
            ...
            <TableRow>
              ...
              <TableCellDiv className="head">
                <THDiv>
                  ...
                  <SortIcon
                    onClick={() =>
                      handleSorting("0-2", "number", "0-2_asc", "0-2_desc", "")
                    }
              ...
              <TableCellDiv className="head">
                <THDiv>
                  ...
                  <SortIcon
                    onClick={() =>
                      handleSorting("2-3", "number", "2-3_asc", "2-3_desc", "")
                    }
                    className="short-width"
                  >
                    <img src={require("../../../Assets/images/sortIcon.png")} />
              ...
              <TableCellDiv className="head">
                <THDiv>
                  ...
                  <SortIcon
                    onClick={() =>
                      handleSorting("3-4", "number", "3-4_asc", "3-4_desc", "")
                    }
                    className="short-width"
                  >
                    <img src={require("../../../Assets/images/sortIcon.png")} />
              ...
              <TableCellDiv className="head">
                <THDiv>
                  ...
                  <SortIcon
                    onClick={() =>
                      handleSorting("4-6", "number", "4-6_asc", "4-6_desc", "")
                    }
                    className="short-width"
                  >
                    <img src={require("../../../Assets/images/sortIcon.png")} />
              ...
              <TableCellDiv className="head">
                <THDiv>
                  ...
                  <SortIcon
                    onClick={() =>
                      handleSorting("6-8", "number", "6-8_asc", "6-8_desc", "")
                    }
                    className="short-width"
                  >
                    <img src={require("../../../Assets/images/sortIcon.png")} />
              ...
              <TableCellDiv className="head">
                <THDiv>
                  ...
                  <SortIcon
                    onClick={() =>
                      handleSorting(">8", "number", ">8_asc", ">8_desc", "")
                    }
                    className="short-width"
                  >
                    <img src={require("../../../Assets/images/sortIcon.png")} />
              ...
              props?.data?.results.map((row, index) => (
                ...
                >
                  <TableCellDiv className="item">
                    {" "}
                    <p>{(props.page - 1) * props.rowsperpage + index + 1}</p>
                  </TableCellDiv>
                  <TableCellDiv className="item">
                    {" "}
                    <p>{row.name}</p>
                  ...
                  <TableCellDiv className="item">
                    {" "}
                    <LinkItem onClick={() => handleOpen(row.id, 0, 7200)}>
                      {row.zero_two}
                    </LinkItem>
                  </TableCellDiv>
                  <TableCellDiv className="item">
                    {" "}
                    <LinkItem onClick={() => handleOpen(row.id, 7200.108)}>
                      {row.two_three}
                    </LinkItem>
                  </TableCellDiv>
                  <TableCellDiv className="item">
                    {" "}
                    <LinkItem onClick={() => handleOpen(row.id, 10800, 14400)}>
                      {row.three_four}
                    </LinkItem>
                  </TableCellDiv>
                  <TableCellDiv className="item">
                    {" "}
                    <LinkItem onClick={() => handleOpen(row.id, 14400, 21600)}>
                      {row.four_six}
                    </LinkItem>
                  </TableCellDiv>
                  <TableCellDiv className="item">
                    {" "}
                    <LinkItem onClick={() => handleOpen(row.id, 21600, 28800)}>
                      {row.six_eight}
                    </LinkItem>
                  </TableCellDiv>
                  <TableCellDiv className="item">
                    {" "}
                    <LinkItem onClick={() => handleOpen(row.id, 28800)}>
                      {row.eight_greater}
                    </LinkItem>
                  </TableCellDiv>
                  <TableCellDiv className="item">
      ...
      <TableFooter>
        <ExportExcel
          fileName="TotalTATDistrict.xlsx"
          source="TOTAL_TAT_DISTRICT"
