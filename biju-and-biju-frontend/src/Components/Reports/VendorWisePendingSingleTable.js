...
import { fetchExcelAgentWisePendingVendorWiseReport } from "../../Functions/helper";
import { ExportExcel } from "../ExportExcel";

export default function VendorWisePendingSingleTable(props) {
  ...
  const handleNavigate = (row) => {
    console.log("row: ", row);
    navigate(
      `/dashboard/vendorwise-pending/details/${row.id}/${encodeURIComponent(
        row.name
      )}/${props.vendor_id}`
    );
  };
  const handleClick = () => {
    exportToExcel(
      fetchExcelAgentWisePendingVendorWiseReport(props?.data?.results),
      "vendor_wise"
    );
  };
  return (
    <>
      ...
      <TableFooter>
        <ExportExcel
          fileName="vendor_wise_district.xlsx"
          source="VENDOR_WISE_DISTRICT"
