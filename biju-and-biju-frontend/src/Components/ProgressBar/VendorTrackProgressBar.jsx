...
function VendorTrackProgressBar(props) {
  ...
  useEffect(() => {
    ...
    socket.addEventListener("message", (event) => {
      async function fetchDownloadAPI() {
        try {
          let url_download = vendor_track_dwonload_file + props.task_id;
          ...
          if (result.status === 200) {
            ...
            link.href = url;
            link.setAttribute("download", "vendor_track.xlsx");
...
export default VendorTrackProgressBar;

const ProgressContainer = styled.div`
  width: ${({ is_list }) => (is_list ? "34%" : "95%")};
  padding: 10px;
`;
