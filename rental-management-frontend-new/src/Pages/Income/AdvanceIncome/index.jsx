...
import { advanceIncomeDownloadApiCall } from '../../../Static/Apis'
import { ErrorToast } from '../../../Utils/AlertMessages'
import AddProperty from './AddProperty'
import { debounce } from '../../../Utils/utils'
import AdvanceIncomeTable from './AdvanceIncomeTable'
import { useAdvanceIncomeListQuery } from '../../../ApiQuery/AdvanceIncome/Queries'
...
const AdvanceIncome = () => {
  ...
  const { data, isLoading, isFetching } = useAdvanceIncomeListQuery(tableParams)
  ...
  const handleDownload = async () => {
    try {
      const fetchAPI = await advanceIncomeDownloadApiCall(tableParams)
    ...
    <PageLayout>
      <div className='flex xl:flex-row  flex-col gap-4 items-center justify-between pb-6'>
        ...
        <div className='w-full gap-2  flex sm:flex-row flex-col sm:w-auto'>
          ...
          <AddButton img={plus} label="Add Advance"
            onClick={() => {
              setId(false)
      ...
      <AdvanceIncomeTable tableParams={{ ...tableParams.pagination, total: data?.count }} data={data?.results} onChange={onChange}
        loading={isLoading || isFetching} setId={setId} id={id} />
    </PageLayout>
  )
}

export default AdvanceIncome